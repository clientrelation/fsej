...
if ( ! function_exists('dw_focus_customize_controls_scripts')) {
    function dw_focus_customize_controls_scripts() {
		...
		wp_enqueue_script( 'validate_jquery', '//ajax.aspnetcdn.com/ajax/jquery.validate/1.13.1/jquery.validate.min.js' );
		wp_enqueue_script( 'additional_methods', '//ajax.aspnetcdn.com/ajax/jquery.validate/1.13.1/additional-methods.min.js' );
...
if( ! function_exists('dw_focus_post_actions') ) {
    ...
    ?>
        <div class="entry-action">
            ...
            <?php if ( is_single() ): ?>
                <div class="co-author" id="noauthor">
                <span class="title-action"><?php _e('Authors','dw_focus') ?></span>
                <?php if( function_exists( 'coauthors_posts_links' ) ) : ?>

                    <?php $coauthors = get_coauthors(); ?>
                    <?php if($coauthor->display_name != "No Author"){?>
                    <?php foreach( $coauthors as $coauthor ): ?>
                        <div class="author-info" id="noauthorinfo">
                            ...
                            <?php endif; ?>
                            <a id="noauthorinfoa" class="author-name" href="<?php echo esc_url( get_author_posts_url($coauthor->ID,$coauthor->user_nicename) ); ?>" rel="author"><?php echo $coauthor->display_name ?></a>
                        </div><!-- .author-info -->
                    <?php endforeach; }?>
            ...
            <div class="tag-action">
                <span class="title-action"><?php _e('TAG CLOUD','dw_focus') ?></span>
...
//newsletter
add_action( 'init', 'codex_newsletter_init' );
/**
 * Register a newsletter  post type.
 *
 * @link http://codex.wordpress.org/Function_Reference/register_post_type
 */
function codex_newsletter_init() {
	$labels = array(
		...
		'all_items'          => __( 'All Newsletter', 'your-plugin-textdomain' ),
	...
	$args = array(
		...
		'show_in_menu'       => true,
		'query_var'          => true,
		'rewrite'            => array( 'slug' => 'news-letter' ),
...
function codex_banner_init() {
	$labels = array(
		'name'               => _x( 'Ad banners', 'post type general name', 'your-plugin-textdomain' ),
		'singular_name'      => _x( 'Adbanner', 'post type singular name', 'your-plugin-textdomain' ),
		'menu_name'          => _x( 'Adbanner', 'admin menu', 'your-plugin-textdomain' ),
		'name_admin_bar'     => _x( 'Adbanner', 'add new on admin bar', 'your-plugin-textdomain' ),
		'add_new'            => _x( 'Add New', 'adbanner', 'your-plugin-textdomain' ),
		'add_new_item'       => __( 'Add New adbanner', 'your-plugin-textdomain' ),
		'new_item'           => __( 'New adbanner', 'your-plugin-textdomain' ),
		'edit_item'          => __( 'Edit adbanner', 'your-plugin-textdomain' ),
		'view_item'          => __( 'View adbanner', 'your-plugin-textdomain' ),
		'all_items'          => __( 'All adbanner', 'your-plugin-textdomain' ),
		'search_items'       => __( 'Search adbanner', 'your-plugin-textdomain' ),
		'parent_item_colon'  => __( 'Parent adbanner:', 'your-plugin-textdomain' ),
		'not_found'          => __( 'No adbanner found.', 'your-plugin-textdomain' ),
		'not_found_in_trash' => __( 'No adbanner found in Trash.', 'your-plugin-textdomain' )
	);
	$args = array(
		'labels'             => $labels,
		'public'             => true,
	...
	register_post_type( 'adbanner', $args );
}
// hook into the init action and call create_book_taxonomies when it fires
add_action( 'init', 'create_bannertype_taxonomies', 0 );

// create two taxonomies, genres and writers for the post type "book"
function create_bannertype_taxonomies() {
	...
	$args = array(
		...
		'query_var'         => true,
		'rewrite'           => array( 'slug' => 'bannertype' ),
	);

	register_taxonomy( 'bannertype', array( 'adbanner' ), $args );
}
add_action( 'edit_form_top', 'top_form_edit' );

function top_form_edit( $post ) {
    if( 'newsletter' == $post->post_type )
        echo "<a href='#' class=".'add-new-h2'." id='test_send'>Test Newsletter</a>";
}
add_action( 'edit_form_top', 'top_form_edit2' );

function top_form_edit2( $post ) {
    if( 'newsletter' == $post->post_type )
        echo "<a href='#' class=".'add-new-h2'." id='force_send'>Force Send Newsletter</a>";
		echo "<div style='display: none;' class='message' id='message'></div>";
}
add_action( 'in_admin_footer', 'manipulate_dom' );
add_action( 'in_admin_footer', 'manipulate_dom2' );
function manipulate_dom()
{
    // Not our post type, exit earlier
    // Adjust post type
    if( 'newsletter' != get_current_screen()->post_type )
        return;
    ?>
...
function manipulate_dom2()
{
    ...
    <script type="text/javascript">
        jQuery(document).ready(function(){
jQuery('#force_send').click(function()
{       
